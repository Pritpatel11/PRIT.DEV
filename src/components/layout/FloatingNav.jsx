...
const FloatingNav = () => {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-max z-[100] flex h-16 items-end gap-2 rounded-2xl glass border border-white/10 dark:border-white/10 bg-black/60 px-4 pb-3 shadow-[0_4px_30px_rgba(0,0,0,0.5)]"
    ...
    return (
        <Link to={item.path}>
            <motion.div
                ref={ref}
                style={{ width }}
                className={`relative aspect-square flex flex-col items-center justify-center rounded-2xl ${isActive ? 'bg-cyber-blue shadow-[0_0_15px_rgba(0,242,255,0.5)]' : 'bg-white/5 hover:bg-white/10 glass'} transition-colors duration-200 group`}
            >
